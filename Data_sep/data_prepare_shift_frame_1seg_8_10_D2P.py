...
import math
import os
file_text = 'realtime_log_ros_20251108_214407D'
file_name = file_text+'.csv'
file_zero = 'realtime_log_ros_20251108_214407D.csv'
file_save = 'D2Presult'+file_text+'2.npz'
file_save_norm = 'D2Presult'+file_text+'_norm.npz'
file_save_csv = 'D2Presult'+file_text+'.csv'
...
def print_last_zero_pressure_positions(csv_file):
    ...
    zero_mask = (df[['p4','p5','p6']].fillna(0) == 0).all(axis=1)
...
def build_shifted_YZ_features_dataset(
    ...
    T_0C_local_zero = np.linalg.inv(T_0B_global_zero) @ T_0C_global_zero
    ...
    for i, row in df_last.iterrows():
        try:
            ...
            # if any(pd.isna(row[f'0C_pos_{axis}']) or row[f'0C_pos_{axis}'] == '' for axis in ['x', 'y']):
                ...
                # continue
            p_tar = np.array([row['p4'], row['p5'], row['p6']], dtype=np.float64)
            ...
            T_0C_local = np.linalg.inv(T_0B_global) @ T_0C_global #distal local
    ...
    df_inputs = pd.DataFrame(inputs, columns=['P4','P5','P6'])
    df_targets = pd.DataFrame(targets, columns=['X','Y'])
