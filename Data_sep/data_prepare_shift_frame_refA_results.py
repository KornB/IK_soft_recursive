...
import math
import os
file_text = 'realtime_log_PDresultrealtime_log_ros_20251108_205609Pshape_pressure_GT_2_7_0_5cableload5'
file_name = file_text+'.csv'
file_zero = 'realtime_log_ros_20251108_205609P.csv'
file_save = 'P2Dresult'+file_text+'2.npz'
file_save_norm = 'P2Dresult'+file_text+'_norm.npz'
file_save_csv = 'P2Dresult'+file_text+'.csv'
...
def build_shifted_YZ_features_dataset(
    ...
    df = pd.read_csv(csv_file)
    mask = (df["pressure"] != df["pressure"].shift(-1)) | (df.index == df.index[-1])
    mask = mask.shift(-2).fillna(False)
    df_last = df[mask].copy().reset_index(drop=True)
    # print(df_last)
    inputs = []
    ...
    T_0C_local_zero = np.linalg.inv(T_0A_global_zero) @ T_0C_global_zero
    ...
    for i, row in df_last.iterrows():
        try:
            ...
            if any(pd.isna(row[f'0C_pos_{axis}']) or row[f'0C_pos_{axis}'] == '' for axis in ['x', 'y']):
                ...
                continue
            raw = row['pressure']
            # 1) strip off the square‐brackets
            clean = raw.strip().lstrip('[').rstrip(']')
            # 2) split on whitespace
            parts = clean.split()
            # 3) convert to floats
            try:
                pressures = np.array([float(x) for x in parts], dtype=np.float64)
            except ValueError:
                print(f"Could not parse pressure string: {raw!r}")
                continue
            pressures = np.pad(pressures, (0, 6 - len(pressures)))[:6]
            # print(pressures[0:3])
            p_tar = (pressures[0:6])
            ...
            current_features = np.concatenate([relP_xz[0:2],[cosP_phi,sinP_phi],relD_xy[0:2],[cosD_phi,sinD_phi]])
    ...
    df_inputs = pd.DataFrame(inputs, columns=['PX','PZ','cosP','sinP','DX','DY','cosD','sinD','dPX','dPZ','dcosP','dsinP','dDX','dDY','dcosD','dsinD'])
    df_targets = pd.DataFrame(targets, columns=['P1','P2','P3','P4','P5','P6'])
