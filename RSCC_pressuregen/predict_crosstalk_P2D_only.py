...
model_name = 'crosstalkP2D_4_128_LSTM'
norm_name  = 'P2Dresultrealtime_log_ros_20251108_205609P_norm.npz'
...
# Example input pressure (replace with your real command)
T = 100
press_prox = np.tile([20, 10, 0], (T,1))

eta_D = predict_crosstalk(
    press_prox,
    model,
    norm['input_mean'],
    norm['input_std'],
...
df = pd.DataFrame(eta_D, columns=['eta_D_x','eta_D_y'])
df.to_csv(os.path.join(save_dir, 'crosstalk_P2D_only.csv'), index=False)

print("Saved crosstalk_P2D_only.csv")
